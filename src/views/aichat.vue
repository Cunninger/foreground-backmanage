<template>
    <div class="background">
        <div>
            ...
            <el-input v-model="content" @keyup.enter="sendRequest" placeholder="输入内容并按回车发送请求"
                style="position:fixed; bottom: 0; left: 50%; transform: translateX(-50%); width: 300px;">
            ...
            <!-- 弹窗 -->
            <el-dialog title="模型设置" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
                <div>选择模型</div>
...
export default {
    ...
    methods: {
        async sendRequest() {
            try {
                const response = await axios.post('https://ollama.yamazing.cn/v1/chat/completions', {
                    model: this.model,
                    messages: [
                        {
                            role: 'user',
        ...
        streamMessage(message) {
            ...
            const interval = setInterval(() => {
                if (index < message.length) {
                    ...
                    this.messages.push({ role: 'bot', content: this.currentMessage });
